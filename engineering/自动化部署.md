...
> 本文档已更新于 [【前端橘子君】](http://xiaoysosheng.top/#/javascript/自动化部署) [【Github】](https://github.com/xiaoyaosheng-yu/library/blob/master/javascript/%E5%8F%98%E9%87%8F%E6%8F%90%E5%8D%87%E5%8F%8A%E4%BC%98%E5%85%88%E7%BA%A7.md)
...
![下载jenkins](http://localhost:5000/images/jenkins/01.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/02.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/06.png)

2、在打开的网站中找到对应文件，然后将文件中的内容复制到管理员密码输入框中，点击"继续"。

![下载jenkins](http://localhost:5000/images/jenkins/07.png)

3、在自定义Jenkins中选择安装推荐的插件

![下载jenkins](http://localhost:5000/images/jenkins/08.png)

4、填写管理员信息并保存（都是必填选项），然后一直点击下一步到配置页面。

![下载jenkins](http://localhost:5000/images/jenkins/09.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/10.png)

2、选择选择"`可选插件`"，然后输入 `node` 进行搜索，选择 `NodeJS Plugin` 插件，点击直接安装，由于我这里已经安装过了，所以没有在可选插件中显示。

![下载jenkins](http://localhost:5000/images/jenkins/11.png)

3、返回`首页`，点击 `Manage Jenkins` 选项，然后点击 `Global Tool Configuration` ，对node进行配置。

![下载jenkins](http://localhost:5000/images/jenkins/12.png)

4、在 `NodeJS` 选项中新增NodeJS，填写好别名以及选择需要安装的NodeJS的版本，最后点击保存按钮。

![下载jenkins](http://localhost:5000/images/jenkins/13.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/14.png)

3、点击 `jenkins` 首页的 `新建Items`。

![下载jenkins](http://localhost:5000/images/jenkins/15.png)

4、给 items 取一个名称，选择 `FreesStyle project` ，然后点击底部的确定按钮。

![下载jenkins](http://localhost:5000/images/jenkins/16.png)

5、在"`源码管理`"中选择`Git`，并输入你刚刚创建的仓库地址。

![下载jenkins](http://localhost:5000/images/jenkins/17.png)

6、在构建环境中勾选 `Provide Node & npm bin/ folder to PATH` 选项，并选择刚刚配置好的 node 配置。点击保存。

![下载jenkins](http://localhost:5000/images/jenkins/18.png)

7、在"`构建`"中选择"`新增构建步骤`"，然后选择 `execute windows batch command`，如果是linux系统需要选择 `execute shell`。

![下载jenkins](http://localhost:5000/images/jenkins/19.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/20.png)

10、返回到首页就可以看到我们刚刚创建的Items了，点击名字后的三角形，然后在下拉框中选中`Build Now`，这个时候会进行第一次构建了。

![下载jenkins](http://localhost:5000/images/jenkins/21.png)

11、点击构建执行状态中的构建任务就可以看到相关进度了，或者点击刚刚创建的Items的名称，然后查看最新的构建历史也可以看到相关进度。

![下载jenkins](http://localhost:5000/images/jenkins/22.png)
![下载jenkins](http://localhost:5000/images/jenkins/23.png)

12、控制台输入日志中如果提示seccess则为构建成功，否则就是失败了。

![下载jenkins](http://localhost:5000/images/jenkins/24.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/25.png)

15、再构建一次，应该没问题了，构建完成后，我们发现在静态服务器资源目录中有我们刚刚构建好的静态文件，我们用node构建一个服务器，来看一下。

![下载jenkins](http://localhost:5000/images/jenkins/26.png)

16、在dist同级目录中打开命令行工具，然后依次输入 `npm init -y` 和 `npm install express` 命令。

![下载jenkins](http://localhost:5000/images/jenkins/27.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/28.png)

18、在命令行工具中输入`node server.js`，然后在浏览器中访问`http://localhost:9999` 就可以看到我们打包后的页面了。

![下载jenkins](http://localhost:5000/images/jenkins/29.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/30.png)

2、返回`jenkins首页`，点击`新建Items`，然后填写一个别名并选择`流水线`，点击确定。

![下载jenkins](http://localhost:5000/images/jenkins/31.png)

3、在构建触发器中选择"`触发远程构建`"，并输入刚刚添加的token。

![下载jenkins](http://localhost:5000/images/jenkins/32.png)

4、在流水线中依次选择 `Pipeline script from SCM` 和 `Git`  ，输入自动触发的仓库的地址，如果需要指定分支也可以修改分支名称，默认分支为master ，脚本路径输入 `Jenkinsfile` 。

![下载jenkins](http://localhost:5000/images/jenkins/33.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/34.png)

6、返回首页，在`Manage Jenkins` 中点击 `Configure Global Security` 。在授权策略中勾选"`匿名用户具有可读权限`"，然后点击保存按钮。

![下载jenkins](http://localhost:5000/images/jenkins/35.png)

7、回到 Gitea 仓库，点击"`仓库设置`"。

![下载jenkins](http://localhost:5000/images/jenkins/36.png)

8、在`管理web钩子`中点击`添加钩子`按钮，然后在下拉选项中点击`Gitea`.

![下载jenkins](http://localhost:5000/images/jenkins/37.png)
...
![下载jenkins](http://localhost:5000/images/jenkins/38.png)
